...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True):
    # Note: Adjust transform list for 3D data
    transform_list = [transforms.Normalize((0.5,), (0.5,))]  # Assume single channel for NIfTI
    # Implement or use 3D versions of these transformations if needed
    # Here we assume that transforms.Resize and transforms.RandomCrop need to be replaced with 3D versions
    
    # Example: Custom transforms for 3D data should be implemented
    # transform_list = [RandomCrop3D((height, width, depth))] + transform_list if crop else transform_list
    # transform_list = [Resize3D(new_size)] + transform_list if new_size is not None else transform_list
...
def __write_images(image_outputs, display_image_num, file_name):
    rows = 3
    cols = 4
    for i, img_array in enumerate(image_outputs):
        num_slices = img_array.shape[2]
        slices = []
        for j in range(min(num_slices, display_image_num)):
            slices.append(image_array[:,:,j])
            slice_file_name = os.path.join(file_name, f"{i}_slice_{j}.png")
            save_slice(slices, slice_file_name, rows, cols)


def write_2images(image_outputs, display_image_num, image_directory, postfix):
#     grid_file_name = os.path.join(image_directory, f'combined_{postfix}.png')
#     save_slice(image_outputs, grid_file_name, rows, cols)
    n = len(image_outputs)
...
def load_vgg16(model_dir):
    """Load or download the pretrained VGG16 model using PyTorch's model repository."""
    # Check if model directory exists, otherwise create it
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)
    # Path for saved VGG16 model weights
    vgg_weights_path = os.path.join(model_dir, 'vgg16.weight')
    if not os.path.exists(vgg_weights_path):
        # Load VGG16 pretrained model from torchvision (this automatically downloads if needed)
        vgg = models.vgg16(pretrained=True)
        # Save the state dict for future use (optional)
        torch.save(vgg.state_dict(), vgg_weights_path)
    else:
        # Load the saved VGG16 model weights
        vgg = models.vgg16()
...
def vgg_preprocess(batch):
    if batch.dim() != 4:
        raise ValueError("The input batch should be a 4D tensor with shape [N, C, H, W].")
    if batch.size(1) == 1:  # Single channel (grayscale) image
        batch = batch.repeat(1, 3, 1, 1)
    elif batch.size(1) == 3:  # RGB image
        (r, g, b) = torch.chunk(batch, 3, dim=1)
        batch = torch.cat((b, g, r), dim=1)
    else:
        raise ValueError("The image must have 1 or 3 channels. Found: {}".format(batch.size(1)))
    batch = (batch + 1) * 255 * 0.5
    mean = torch.tensor([103.939, 116.779, 123.680]).view(1, 3, 1, 1).cuda()  # Ensure it’s on the same device as input
